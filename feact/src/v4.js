...
let currentRoot = null // 保存commit阶段时的fiber树用于和下一次更新的fiber树做对比
 ...
 *
 * v3.0之前的版本只处理了新增DOM的情况，没有处理更新和删除的情况
 * 更新和删除的情况需要保存上一次commit时的fiber树，我们称之为currentFiberTree
 *
 * 在fiber结构中添加属性alternate用来表示新生成的workInProgressFiber对应的原来currentFiber
 */

//
/**
 * 通过childElements生成childFiber，并建立他们之间的连接，形成fiber树
 * @param {*} parent
 */
function reconcileChildren(parent){
  const childElements = parent.props.children

  let index = 0
  let preSiblingFiber = null
  let oldParentFiber = parent.alternate.child
